...
export const Route = createFileRoute("/diaries_/$slug")({
  head: ({ loaderData }) => {
    const e = loaderData as
      { title?: string; body?: string | null; cover_image_url?: string | null } | undefined;
    if (!e)
      return {
        meta: [{ title: "Entry — Gaijin Diaries" }, { name: "robots", content: "noindex" }],
      };
    return {
      meta: [
        ...
        {(e.chapter_number != null || e.chapter_title || e.part_number != null || e.part_title) && (
          <p className="mt-4 font-mono text-[11px] uppercase tracking-[0.2em] text-white/50">
            {e.chapter_number != null && (
              <>
                {t("detail.chapter", { n: e.chapter_number })}
                {e.part_number != null ? `.${e.part_number}` : ""}
              </>
            )}
